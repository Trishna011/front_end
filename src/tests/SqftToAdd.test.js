...
test('renders the heading', () => {
  ...
  render(<SqftToAdd onNext={mockOnNext} onBack={mockOnBack} answers={answers} />)
...
test('renders the correct placeholder text on inputs', () => {
  ...
  expect(screen.getByPlaceholderText(/Enter sqft to add \(0 if none\)/i)).toBeInTheDocument()
})
...
// Full renovation should show only one input
// ============================================================
...
test('inputs default to empty string', () => {
  const answers = { renovation_type: ['Kitchen'] }
  render(<SqftToAdd onNext={mockOnNext} onBack={mockOnBack} answers={answers} />)
...
// Only digits allowed — letters, decimals, special chars rejected
// ============================================================
...
test('onNext is NOT called when an input is empty', () => {
  const answers = { renovation_type: ['Kitchen'] }
  render(<SqftToAdd onNext={mockOnNext} onBack={mockOnBack} answers={answers} />)
...
test('onNext is NOT called when bedroom filled but bathroom is not', () => {
  const answers = {
...
test('shows error when bedroom filled but bathroom is not', () => {
  const answers = {
    renovation_type: ['Bedroom', 'Bathroom'],
    bedrooms_to_reno: 1,
    bathrooms_to_reno: 1
...
test('calls onNext with correct sqft_to_add for a single other type', () => {
  const answers = { renovation_type: ['Kitchen'] }
  render(<SqftToAdd onNext={mockOnNext} onBack={mockOnBack} answers={answers} />)
...
test('values are passed as numbers not strings', () => {
  ...
  expect(typeof arg.sqft_to_add.other['Kitchen']).toBe('number')
})
...
test('renders without crashing when answers is undefined', () => {
  render(<SqftToAdd onNext={mockOnNext} onBack={mockOnBack} answers={undefined} />)
